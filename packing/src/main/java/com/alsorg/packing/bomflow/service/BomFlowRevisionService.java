...
public class BomFlowRevisionService {
    ...
    public RevisionDetailResponse get(
        ...
        return mapper.toRevisionDetail(
                requireRevisionWithProduct(revisionId));
    }

    public RevisionDetailResponse addLine(
            UUID revisionId,
            RevisionLineRequest request) {

        access.requireEditor();

        BomFlowRevision revision = requireRevisionForUpdate(
                revisionId);

        requireEditable(revision);
        validateLineRequest(request);

        int nextLineNo = itemRepository
                .findTopByRevisionIdOrderByLineNoDesc(revision.id)
    ...
    public RevisionDetailResponse updateLine(
        ...
        BomFlowRevisionItem item = itemRepository
    ...
    public RevisionDetailResponse submit(
        ...
        if (!(revision.status == BomFlowRevisionStatus.DRAFT
                || revision.status == BomFlowRevisionStatus.RETURNED)) {

            throw badRequest(
                    "Only Draft or Returned revisions can be submitted.");
        }
        ...
        if (items.isEmpty()) {
            throw badRequest(
                    "Add at least one BOM row before submission.");
        }

        for (BomFlowRevisionItem item : items) {
            if (zero(item.requiredQty)
                    .compareTo(BigDecimal.ZERO) <= 0) {

                throw badRequest(
                        "Every BOM row must have quantity greater than zero.");
            }
            ...
            if (zero(item.rate)
                ...
                throw badRequest(
                        "Complete all missing material rates before submission.");
            }
        }
        ...
        revision.remarks = firstNonBlank(
                request == null
                        ? null
                        : request.remarks(),
                revision.remarks);
    ...
    public RevisionDetailResponse verify(
        ...
        if (revision.status != BomFlowRevisionStatus.SUBMITTED) {
            throw badRequest(
                    "Only Submitted revisions can be verified.");
        }
    ...
    public RevisionDetailResponse returnForCorrection(
        ...
        if (!(revision.status == BomFlowRevisionStatus.SUBMITTED
            ...
            throw badRequest(
                    "Only Submitted or Verified revisions can be returned.");
        }
    ...
    public RevisionDetailResponse approve(
        ...
        if (revision.status != BomFlowRevisionStatus.VERIFIED) {
            throw badRequest(
                    "Only Verified revisions can be approved.");
        }
    ...
    private void applyLineRequest(
        ...
        BigDecimal quantity = request.requiredQty() != null
                ? request.requiredQty()
                : request.quantity();

        BigDecimal gst = request.gstPercent() != null
                ? request.gstPercent()
                : request.gst();
    ...
    private void validateLineRequest(
        ...
        if (request == null) {
            throw badRequest(
                    "BOM row request body is required.");
        }
    }
    ...
    private void requireEditable(
        ...
        if (!(revision.status == BomFlowRevisionStatus.DRAFT
            ...
            throw badRequest(
                    "Only Draft or Returned revisions can be edited.");
        }
    }

    private BomFlowRevision requireRevisionForUpdate(
            UUID revisionId) {
        ...
        return revisionRepository
                .findByIdForUpdate(revisionId)
                .orElseThrow(() -> notFound(
                        "BOM revision not found: "
                                + revisionId));
    }

    private BomFlowRevision requireRevisionWithProduct(
            UUID revisionId) {
        ...
        return revisionRepository
                .findByIdWithProduct(revisionId)
                .orElseThrow(() -> notFound(
                        "BOM revision not found: "
                                + revisionId));
    }
