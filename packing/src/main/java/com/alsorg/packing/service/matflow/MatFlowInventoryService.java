...
public class MatFlowInventoryService {
    ...
    public List<LocationResponse> listLocations(
        ...
        return locationRepository
                .findByPlantCodeInAndActiveTrueOrderByLocationCodeAsc(
    ...
    private void applyLocation(
            ...
            LocationRequest request) {
        location.locationCode = upper(request.locationCode());

        location.locationName = clean(request.locationName());

        location.plantCode = upper(request.plantCode());

        location.locationType = request.locationType();

        location.ownershipType = request.ownershipType() == null
                ? OwnershipType.INTERNAL
                : request.ownershipType();

        location.supportsStock = request.supportsStock() == null ||
                request.supportsStock();

        location.address = clean(request.address());

        location.contactPerson = clean(request.contactPerson());

        location.contactPhone = clean(request.contactPhone());
