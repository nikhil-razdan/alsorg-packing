...
const SERVICE_DOMAINS = [
  ["MACHINE", "Machine / Production", "Machine breakdown, abnormal sound, motion, tooling or production equipment"],
  ["IT", "IT / Computer", "PC, laptop, printer, LAN, internet, software, peripheral or network support"],
  ["ELECTRICAL", "Electrical", "Lights, wiring, sockets, panels, electrical faults and power issues"],
  ["FACILITY", "Facility", "AC, civil, doors, plumbing, furniture and office/factory facility maintenance"],
  ["UTILITY", "Utility", "Compressor, DG, water, air, utility equipment and common services"],
  ["GENERAL", "General Maintenance", "Other controlled maintenance or repair support"],
];

const CATEGORY_OPTIONS = {
  MACHINE: ["Breakdown", "Abnormal noise/vibration", "Electrical fault", "Pneumatic/Hydraulic", "Tooling/Alignment", "Safety issue", "Other"],
  IT: ["PC/Laptop", "LAN/Internet", "Printer/Scanner", "Software/Login", "Email/Network", "Peripheral", "Other"],
  ELECTRICAL: ["Light", "Wiring", "Socket/Power", "Panel/MCB", "Motor/Electrical", "Safety issue", "Other"],
  FACILITY: ["AC/Cooling", "Plumbing", "Door/Furniture", "Civil", "Housekeeping support", "Other"],
  UTILITY: ["Compressor/Air", "DG/Power backup", "Water", "Pump", "Utility line", "Other"],
  GENERAL: ["Repair", "Inspection", "Service", "Safety issue", "Other"],
};
      ...
      >
        ...
        <small>
          {sessionUser
            ...
            : "For employees who already have a FlowSuite login in any module."}
        </small>
      </button>
...
function RequestForm({
  ...
  const allowedDomains = identity === "REPORTER"
    ...
    : SERVICE_DOMAINS.map(([key]) => key);
  const desks = identity === "REPORTER"
    ? reporterContext?.serviceDesks || []
    : flowContext?.serviceDesks || [];
  const plants = identity === "REPORTER"
    ? [reporterContext?.reporter?.plantCode].filter(Boolean)
    : flowContext?.plants || [];

  const initialDomain = asset?.serviceDomain || tokenDesk?.serviceDomain || allowedDomains[0] || "GENERAL";
  const initialPlant = asset?.plantCode || tokenDesk?.plantCode || reporterContext?.reporter?.plantCode || (plants.length === 1 ? plants[0] : "");
  ...
  useEffect(() => {
    setForm((current) => ({
      ...
      plantCode: asset?.plantCode || tokenDesk?.plantCode || reporterContext?.reporter?.plantCode || current.plantCode || initialPlant,
      location: asset?.location || current.location,
      serviceDeskToken: tokenDesk?.token || current.serviceDeskToken,
    }));
  }, [asset?.serviceDomain, asset?.plantCode, asset?.location, tokenDesk?.serviceDomain, tokenDesk?.plantCode, tokenDesk?.token, reporterContext?.reporter?.plantCode]);
  ...
  const plantLocked = identity === "REPORTER" || Boolean(asset?.plantCode || tokenDesk?.plantCode);
  const categories = selectedDesk?.categories?.length
    ? selectedDesk.categories
    : CATEGORY_OPTIONS[form.serviceDomain] || CATEGORY_OPTIONS.GENERAL;
    ...
    <div className="mf-shell mf-request-shell">
      ...
      <main className="mf-request-main">
        <section className="mf-request-hero">
          ...
          <h1>One place for machine, IT, electrical and facility support.</h1>
          <p>
              ...
              {mode === "REPORTER" && reporterContext && (
                <div className="mf-request-verified">
                  ...
                  <div>
                    ...
                    <small>{[reporterContext.reporter?.reporterCode, reporterContext.reporter?.department, reporterContext.reporter?.plantCode].filter(Boolean).join(" · ")}</small>
                  </div>
        ...
        <section className="mf-request-policy">
          ...
          <p>
            FlowSuite users are identified by their normal account. Other approved employees use a Reporter Pass created by Maintenance/Admin. A Reporter Pass is plant-scoped, service-scoped, can be disabled or expired, and is PIN protected. Scanning a QR only identifies the asset or service desk; it never grants permission by itself.
          </p>
        </section>
      </main>
    </div>
