...
function AccessChoice({ sessionUser, mode, onMode }) {
  return (
    <div className="af-request-access-grid">
      <button
        type="button"
        className={`af-request-access-card ${mode === "FLOW_SUITE" ? "is-active" : ""}`}
        onClick={() => onMode("FLOW_SUITE")}
      >
        <span className="af-request-access-icon">FS</span>
        <strong>FlowSuite employee</strong>
        <small>
          {sessionUser
            ? `Continue as ${sessionUser}`
            : "For approved employees whose FlowSuite username is linked to a Reporter profile, plus operational AssetFlow staff."}
        </small>
      </button>

      <button
        type="button"
        className={`af-request-access-card ${mode === "REPORTER" ? "is-active" : ""}`}
        onClick={() => onMode("REPORTER")}
      >
        <span className="af-request-access-icon">RP</span>
...
function ContextCard({ asset, desk }) {
  ...
  return (
    <div className="af-request-context-card">
      <div className="af-request-context-mark">{asset ? "A" : "D"}</div>
      <div>
...
function RequestForm({
  ...
  return (
    <form className="af-request-form" onSubmit={submit}>
      <div className="af-request-section-head">
        <div>
          <span>REQUEST DETAILS</span>
          <h2>Tell the right team what needs attention</h2>
        </div>
        <div className="af-request-security-chip">Controlled identity</div>
      </div>

      <div className="af-request-form-grid">
        ...
        <label className="af-field">
          <span className="af-label">Request category *</span>
          <select
            required
        ...
        <div className="af-request-route-banner af-full">
        ...
        {!tokenDesk && desks.length > 0 && !asset && (
          <label className="af-field af-full">
            <span className="af-label">Service desk / team</span>
            <select
        ...
        <label className="af-field">
          <span className="af-label">Plant / Site *</span>
        ...
        <label className="af-field">
          <span className="af-label">Exact location</span>
          <input
            value={form.location}
        ...
        <label className="af-field af-full">
          <span className="af-label">Issue / request title *</span>
          <input
            required
            maxLength={300}
            value={form.title}
        ...
        <label className="af-field af-full">
          <span className="af-label">Problem description *</span>
          <textarea
            required
            rows={5}
        ...
        <label className="af-field">
          ...
          <span className="af-hint">Safety/production-stop flags can automatically escalate it.</span>
        </label>

        <div className="af-request-flags af-full">
          <label>
      ...
      <div className="af-request-submit-bar">
        <div>
        ...
        <button className="af-btn af-btn-primary" type="submit" disabled={submitting}>
...
function AssetFlowRequestPortalContent() {
  ...
  const authorise = async (event) => {
    ...
    setMessage(null);
    try {
      const response = await assetFlowApi.publicAuthorise({
        ...credentials,
      ...
      try {
        const mine = await assetFlowApi.publicMyRequests({
  ...
  const submitRequest = async (form) => {
    ...
    try {
      ...
      if (mode === "FLOW_SUITE") {
        ...
        response = await assetFlowApi.createRequesterRequest({
          equipmentId: null,
          equipmentToken: assetToken || null,
        ...
        try {
          const mine = await assetFlowApi.myRequests();
          setRecentRequests(Array.isArray(mine) ? mine : []);
        } catch {
        ...
        response = await assetFlowApi.publicCreateRequest({
          reporterCode: credentials.reporterCode,
          accessPin: credentials.accessPin,
          equipmentToken: assetToken || null,
          serviceDeskToken: form.serviceDeskToken || deskToken || null,
      ...
      setResult(response || {});
      setMessage({ type: "success", text: "Your request has been submitted to AssetFlow." });
  ...
  if (loading) {
    return <div className="af-shell af-request-shell"><div className="af-loading"><span /><span /><span /></div></div>;
  }

  return (
    <div className="af-shell af-request-shell">
      <header className="af-request-header">
        <div className="af-request-brand">
      ...
      <main className="af-request-main">
        <section className="af-request-hero">
          <span className="af-eyebrow">CONTROLLED SERVICE REQUEST</span>
        ...
        {message && <div className={`af-request-message ${message.type === "error" ? "is-error" : "is-success"}`}>{message.text}</div>}

        {result && (
          <section className="af-request-success-card">
            <span>REQUEST REGISTERED</span>
            <strong>{result.workNumber || "AssetFlow request"}</strong>
            ...
            <button type="button" className="af-btn" onClick={() => setResult(null)}>Raise another request</button>
          </section>
        )}

        {!result && (
          <>
            <section className="af-request-card">
              <div className="af-request-section-head">
              ...
              {mode === "FLOW_SUITE" && sessionUser && (
                <div className="af-request-verified">
                  <span>✓</span><div><strong>{sessionUser}</strong><small>Authenticated FlowSuite employee</small></div>
                </div>
              )}

              {mode === "REPORTER" && !reporterContext && (
                <form className="af-request-pin-form" onSubmit={authorise}>
                  <label className="af-field"><span className="af-label">Reporter / Employee Code</span><input required autoComplete="username" value={credentials.reporterCode} onChange={(e) => setCredentials((c) => ({ ...c, reporterCode: e.target.value }))} placeholder="Example: EMP-0142" /></label>
                  <label className="af-field"><span className="af-label">Reporter PIN</span><input required type="password" inputMode="numeric" autoComplete="current-password" minLength={4} maxLength={8} value={credentials.accessPin} onChange={(e) => setCredentials((c) => ({ ...c, accessPin: e.target.value.replace(/\D/g, "") }))} placeholder="4–8 digit PIN" /></label>
                  <button className="af-btn af-btn-primary" disabled={authorising}>{authorising ? "Verifying…" : "Verify Reporter Pass"}</button>
                </form>
              )}
              ...
              {mode === "REPORTER" && reporterContext && (
                <div className="af-request-verified">
                  ...
                  <button type="button" className="af-btn" onClick={() => setReporterContext(null)}>Change</button>
                </div>
              )}
            </section>
        ...
        {((mode === "FLOW_SUITE" && sessionUser) || (mode === "REPORTER" && reporterContext)) && recentRequests.length > 0 && (
          <section className="af-request-card">
            <div className="af-request-section-head"><div><span>MY REQUESTS</span><h2>Recent requests raised by you</h2></div><small>Last {Math.min(10, recentRequests.length)}</small></div>
            <div className="af-request-history">
        ...
        <section className="af-request-policy">
          <strong>How access works</strong>
          <p>
            A FlowSuite account can raise a request only when Admin has linked that username to an approved AssetFlow Reporter profile (or the account is operational AssetFlow staff). Employees without FlowSuite use a Reporter Pass with authorised plants and departments. Scanning a QR only identifies the exact machine, IT asset or service desk; it never grants permission by itself.
          </p>
        </section>
      </main>
    </div>
...
export default function AssetFlowRequestPortal() {
  return (
    <AssetFlowThemeProvider>
      <AssetFlowRequestPortalContent />
    </AssetFlowThemeProvider>
  );
}
