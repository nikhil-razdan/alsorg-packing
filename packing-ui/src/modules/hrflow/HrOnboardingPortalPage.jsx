				...
				<>
					<Paper sx={{ ...panelSx, p: 2.2, mb: 2 }}>
						<Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "1.1fr .9fr" }, gap: 2 }}>
							...
							<Box sx={{ p: 1.7, borderRadius: 1.7, background: "#f8fafc", border: `1px solid ${hrColors.line}` }}>
					...
					<Paper sx={{ ...panelSx, p: 2.2, mb: 2 }}>
						...
						<Box sx={{ display: "grid", gap: .7, maxHeight: 420, overflowY: "auto", pr: .5 }}>
							{(portal.orientation?.tasks || []).map((task) => <Box key={task.code} sx={{ display: "flex", gap: 1, alignItems: "flex-start", p: 1, borderRadius: 1.2, background: task.completed ? "#f0fdf4" : "#f8fafc", border: `1px solid ${task.completed ? "#bbf7d0" : hrColors.line}` }}><TaskAltOutlinedIcon sx={{ fontSize: 18, color: task.completed ? hrColors.green : "#94a3b8", mt: .1 }} /><Box><Typography sx={{ fontSize: 12.5, fontWeight: 800, color: hrColors.ink }}>{task.label}</Typography><Typography sx={{ fontSize: 11, color: hrColors.muted }}>{humanize(task.section)} {task.completedBy ? `• ${task.completedBy}` : ""}</Typography></Box></Box>)}
						</Box>
...
function Info({ label, value }) { return <Box sx={{ p: 1, borderRadius: 1.2, background: "#f8fafc", border: `1px solid ${hrColors.line}` }}><Typography sx={{ color: hrColors.muted, fontSize: 10.5, fontWeight: 800 }}>{label}</Typography><Typography sx={{ color: hrColors.ink, fontSize: 12.5, fontWeight: 850, mt: .25 }}>{value || "—"}</Typography></Box>; }
function MiniStatus({ label, done, helper }) { return <Box sx={{ p: 1, borderRadius: 1.3, background: done ? "#ecfdf5" : "#f8fafc", border: `1px solid ${done ? "#a7f3d0" : hrColors.line}` }}><Typography sx={{ fontSize: 11.5, fontWeight: 900, color: done ? hrColors.green : hrColors.ink }}>{label}</Typography><Typography sx={{ mt: .2, fontSize: 10.5, color: hrColors.muted }}>{helper}</Typography></Box>; }

function ActionDocument({ title, accepted, helper, body, actionLabel, onAction, busy }) {
	return <Paper sx={{ ...panelSx, p: 2.2, mb: 2 }}><Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 1, flexWrap: "wrap", mb: 1.5 }}><Box><Typography sx={{ fontSize: 17, fontWeight: 950 }}>{title}</Typography><Typography sx={{ color: hrColors.muted, fontSize: 12.5, mt: .3 }}>{helper}</Typography></Box>{accepted ? <Chip icon={<CheckCircleOutlineOutlinedIcon />} label="Completed" sx={{ color: hrColors.green, background: "#ecfdf5", fontWeight: 850 }} /> : null}</Box>{body}<Box sx={{ mt: 1.5 }}><Button variant="contained" disabled={accepted || busy} onClick={onAction} sx={primaryButtonSx}>{accepted ? "Acknowledged" : actionLabel}</Button></Box></Paper>;
}
...
function LegalCard({ title, snapshot, acceptance, typedName, actionLabel, onAction, busy }) {
	...
	return <Paper sx={{ ...panelSx, p: 2.2, mb: 2 }}><Box sx={{ display: "flex", justifyContent: "space-between", gap: 1, alignItems: "flex-start", flexWrap: "wrap" }}><SectionHeader title={title} subtitle={`Version ${snapshot.version} • Published ${formatDateTime(snapshot.publishedAt)}`} />{acceptance ? <Chip icon={<CheckCircleOutlineOutlinedIcon />} label="Accepted" sx={{ color: hrColors.green, background: "#ecfdf5", fontWeight: 850 }} /> : null}</Box><Paper variant="outlined" sx={{ p: 1.5, borderRadius: 1.5, maxHeight: 340, overflowY: "auto", whiteSpace: "pre-wrap", background: "#fcfdff" }}><Typography sx={{ fontSize: 12.5, lineHeight: 1.7, color: hrColors.ink }}>{snapshot.body}</Typography></Paper><Divider sx={{ my: 1.5 }} /><Button variant="contained" disabled={Boolean(acceptance) || busy || !typedName.trim()} onClick={onAction} sx={primaryButtonSx}>{acceptance ? `Accepted ${formatDateTime(acceptance.acceptedAt)}` : actionLabel}</Button></Paper>;
}
