...
function HrFlowPublicEntryContent() {
	...
	if (mode === "apply" && token) {
		return <HrCandidateApplicationPage token={decodeURIComponent(token)} />;
	}
	if (mode === "onboarding" && token) {
		return <HrOnboardingPortalPage token={decodeURIComponent(token)} />;
	}
	...
	return (
		<Box sx={{ minHeight: "100vh", background: "var(--hr-page-bg)", p: 2, display: "grid", placeItems: "center" }}>
			<Paper sx={{ ...panelSx, p: 3, width: "min(520px,100%)" }}>
				<HrBrand />
				<Typography sx={{ mt: 2.5, fontSize: 22, fontWeight: 950, color: hrColors.ink }}>
					Invalid HRFlow link
				</Typography>
