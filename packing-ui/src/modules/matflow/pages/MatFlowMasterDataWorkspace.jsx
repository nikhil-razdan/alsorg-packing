...
function MasterDialog({
    ...
    const plant = (
        <TextField
            ...
            sx={fieldSx}
        >
            {availablePlants.map((plantCode) => (
                <MenuItem key={plantCode} value={plantCode}>
                    {plantCode}
                </MenuItem>
            ))}
        </TextField>
    ...
    return (
        ...
        >
            <DialogTitle sx={dialogTitleSx}>
                {row ? "Edit" : "Add"} {type === "materials" ? "Material" : "Location"}
            </DialogTitle>
            ...
            <DialogContent sx={dialogContentSx}>
                ...
                >
                    {type === "materials" ? (
                        <>
                            {text("materialCode", "Material Code *")}
                            {text("materialName", "Material Name *")}
                            ...
                            <TextField
                                ...
                                sx={fieldSx}
                            >
                                {MATFLOW_MATERIAL_CATEGORIES.map((category) => (
                            ...
                            {text("uom", "UOM *")}
                            {text("preferredSupplier", "Preferred Supplier")}
                            {text("minimumStock", "Minimum Stock", { type: "number" })}
                            {text("reorderLevel", "Reorder Level", { type: "number" })}
                        ...
                        <>
                            {text("locationCode", "Location Code *")}
                            ...
                            <TextField
                                ...
                                sx={fieldSx}
                            >
                                {metadata.locationTypes.map((value) => (
                            ...
                            <TextField
                                ...
                                sx={fieldSx}
                            >
                                {metadata.ownershipTypes.map((value) => (
                                    <MenuItem key={value} value={value}>
                                        {value}
                                    </MenuItem>
                                ))}
                ...
                <Box sx={{ mt: 1.5, display: "flex", gap: 2, flexWrap: "wrap" }}>
                    {type === "locations" && (
                        <FormControlLabel
                            control={
                                <Switch
                                    checked={form.supportsStock === true}
                                    onChange={(e) =>
                                        setForm((current) => ({
                                            ...current,
                                            supportsStock: e.target.checked,
                                        }))
                                    }
                                />
                            }
            ...
            <DialogActions sx={dialogActionsSx}>
                ...
                <Button onClick={onSave} disabled={saving} sx={primaryBtnSx}>
                    {saving ? "Saving..." : "Save"}
                </Button>
            </DialogActions>
        </Dialog>
...
function MasterPage({ type }) {
    ...
    const [form, setForm] = useState(
        type === "materials" ? emptyMaterial : emptyLocation
    );
    const [metadata, setMetadata] = useState({
        locationTypes: FALLBACK_LOCATION_TYPES,
        ownershipTypes: FALLBACK_OWNERSHIP_TYPES,
    });
    ...
    const api = useMemo(
        () =>
            type === "materials"
                ? {
                    list: matflowApi.listMaterials,
                    create: matflowApi.createMaterial,
                    update: matflowApi.updateMaterial,
                }
                : {
                    list: matflowApi.listLocations,
                    create: matflowApi.createLocation,
                    update: matflowApi.updateLocation,
                },
        [type]
    );
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            setError(
                readMatFlowError(
                    requestError,
                    "Unable to load MatFlow master data."
                )
            );
        } finally {
            setLoading(false);
    ...
    useEffect(() => {
        ...
        matflowApi
            ...
            .then((response) =>
                setMetadata({
                    locationTypes: metadataEnum(
                        response?.data,
                        "locationType",
                        FALLBACK_LOCATION_TYPES
                    ),
                    ownershipTypes: metadataEnum(
                        response?.data,
                        "ownershipType",
    ...
    const blank = type === "materials" ? emptyMaterial : emptyLocation;

    const openCreate = () => {
        setDialog({ row: null });
        setForm({
            ...blank,
            plantCode: blank.plantCode || availablePlants[0] || "",
        });
        setError("");
    };
    ...
    const validate = () => {
        if (type === "materials") {
            if (
                !clean(form.materialCode) ||
                !clean(form.materialName) ||
                !clean(form.category) ||
                !clean(form.uom)
            ) {
                return "Material code, name, category and UOM are required.";
            }
            return "";
        }
        ...
        if (
            ...
            return "Location code, name, plant, type and ownership are required.";
        }
    ...
    const body = () => {
        ...
        return {
            ...
            ownershipType: normalize(form.ownershipType || "INTERNAL"),
    ...
    const save = async () => {
        ...
        try {
            if (dialog?.row?.id) {
                await api.update(dialog.row.id, body());
            } else {
            ...
            setError(
                readMatFlowError(
                    requestError,
                    "Unable to save MatFlow master record."
                )
            );
        } finally {
            setSaving(false);
    ...
    const title =
        type === "materials" ? "Material Master" : "Material Locations";

    const columns =
        type === "materials"
            ? ["Material", "Category", "UOM", "Min / Reorder", "Status", "Action"]
    ...
    const rowCells = (row) => {
        if (type === "materials") {
            return [
                ...
                <MatFlowStatusChip key="status" status={row.active ? "ACTIVE" : "INACTIVE"} />,
                canManage ? (
                    <Button key="edit" onClick={() => openEdit(row)} sx={secondaryBtnSx}>
        ...
        return [
            <Box key="location">
                ...
                <Typography sx={subTextSx}>{row.locationName}</Typography>
            </Box>,
            row.plantCode,
            row.locationType,
            row.ownershipType,
    ...
    return (
        <Box sx={pageSx}>
            <PageHero
                badge="MATFLOW MASTER DATA"
                title={title}
                subtitle={
                    type === "materials"
                        ? "Maintain canonical material identities, categories, units and stock thresholds."
                        : "Maintain Store, QC, Processing, Production and external processing locations used by the physical material workflow."
                }
                actions={
                    canManage ? (
            ...
            <Card sx={panelSx}>
                {loading ? (
                    ...
                    <Box sx={tableShellSx}>
                        <Box sx={{ ...tableHeaderSx, gridTemplateColumns: grid }}>
                            {columns.map((column) => (
                                <Box key={column} sx={tableCellSx}>
                                    {column}
                                </Box>
                            ))}
                        </Box>
                        ...
                        {rows.length === 0 ? (
                            ...
                            rows.map((row) => (
                                <Box
                                    key={row.id}
                                    sx={{ ...tableRowSx, gridTemplateColumns: grid }}
                                >
                                    {rowCells(row).map((cell, index) => (
                                        <Box key={index} sx={tableCellSx}>
                                            {cell}
                                        </Box>
                                    ))}
                                </Box>
...
export function MatFlowProjectsPage() {
    ...
    const saveProject = async () => {
        ...
        try {
            if (projectDialog?.row?.id) await matflowApi.updateProject(projectDialog.row.id, body);
            else await matflowApi.createProject(body);
    ...
    const openProduct = (project, product = null) => {
        ...
        setProductForm(product ? {
            ...
            remarks: product.remarks || "", active: product.active !== false,
    ...
    const saveProduct = async () => {
        ...
        const body = {
            ...
            drawingRevision: upperCode(productForm.drawingRevision) || "0", requiredDate: productForm.requiredDate || null,
    ...
    const decideProduct = async () => {
        ...
        try {
            ...
            if (approval.type === "APPROVE") {
                await matflowApi.approveProjectProduct(approval.project.id, approval.product.id, body);
            } else {
                await matflowApi.returnProjectProduct(approval.project.id, approval.product.id, body);
            }
    ...
    return <Box sx={pageSx}>
                    ...
                    <Box sx={tableShellSx}>
                        ...
                        {(project.products || []).length === 0 ? <EmptyState>No Products have been created under this Project.</EmptyState> : (project.products || []).map((product) => <Box key={product.id} sx={{ ...tableRowSx, gridTemplateColumns: "minmax(220px,1.1fr) 150px 170px 170px 150px minmax(180px,.8fr) 160px minmax(240px,1fr)" }}>
                            ...
                            <Box sx={{ ...tableCellSx, display: "flex", gap: .5, flexWrap: "wrap" }}>
                                ...
                                {normalize(product.approvalStatus) === "APPROVED" && <Button onClick={() => navigate(product.latestBomId ? `/matflow/boms/${product.latestBomId}` : `/matflow/boms/new?productId=${encodeURIComponent(product.id)}`)} sx={secondaryBtnSx}>{product.latestBomId ? "BOM" : "Create BOM"}</Button>}
                                {product.latestRequisitionId && <Button onClick={() => navigate(`/matflow/tracker/${product.latestRequisitionId}`)} sx={primaryBtnSx}>Latest Trace</Button>}
                            </Box>
                        </Box>)}
                    </Box>
                </Box>}
        ...
        <Dialog open={Boolean(projectDialog)} onClose={() => !saving && setProjectDialog(null)} fullWidth maxWidth="md" PaperProps={{ sx: dialogPaperSx }}><DialogTitle sx={dialogTitleSx}>{projectDialog?.row ? "Edit Project" : "Create Client Project"}</DialogTitle><DialogContent sx={dialogContentSx}><Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 1.3 }}>
            <TextField label="Project / PD Code *" value={projectForm.projectCode} disabled={Boolean(projectDialog?.row?.productCount)} helperText={projectDialog?.row?.productCount ? "Project code is locked after Products exist." : ""} onChange={(e) => setProjectForm((c) => ({ ...c, projectCode: e.target.value }))} sx={fieldSx} />
            ...
            <TextField select label="Plant *" value={projectForm.plantCode} disabled={Boolean(projectDialog?.row?.productCount)} helperText={projectDialog?.row?.productCount ? "Plant is locked after Products exist." : ""} onChange={(e) => setProjectForm((c) => ({ ...c, plantCode: e.target.value }))} sx={fieldSx}>{availablePlants.map((plant) => <MenuItem key={plant} value={plant}>{plant}</MenuItem>)}</TextField>
        ...
        </Box><FormControlLabel sx={{ mt: 1 }} control={<Switch checked={projectForm.active !== false} onChange={(e) => setProjectForm((c) => ({ ...c, active: e.target.checked }))} />} label="Project active" /></DialogContent><DialogActions sx={dialogActionsSx}><Button onClick={() => setProjectDialog(null)} sx={secondaryBtnSx}>Cancel</Button><Button onClick={saveProject} disabled={saving} sx={primaryBtnSx}>{saving ? "Saving..." : "Save Project"}</Button></DialogActions></Dialog>
        ...
        </Box><FormControlLabel sx={{ mt: 1 }} control={<Switch checked={productForm.active !== false} onChange={(e) => setProductForm((c) => ({ ...c, active: e.target.checked }))} />} label="Product active" /></DialogContent><DialogActions sx={dialogActionsSx}><Button onClick={() => setProductDialog(null)} sx={secondaryBtnSx}>Cancel</Button><Button onClick={saveProduct} disabled={saving} sx={primaryBtnSx}>{saving ? "Saving..." : "Save Product"}</Button></DialogActions></Dialog>
