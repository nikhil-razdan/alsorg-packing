...
async function discoverProcessingCandidates() {
    ...
    const snapshots = await Promise.all(requisitions.map(async (r) => { try { return (await matflowApi.getRequisitionPlanning(r.id))?.data; } catch { return null; } }));
    ...
    for (const snapshot of snapshots.filter(Boolean)) {
        ...
        (snapshot.reservations || []).forEach((reservation) => {
            const line = (requisition.lines || []).find((l) => String(l.id) === String(reservation.requisitionLineId));
            const step = routes.find((s) => String(s.bomLineId) === String(line?.bomLineId) && normalize(s.stepType) === "PROCESSING");
            if (step && ["ACTIVE", "PARTIALLY_ISSUED"].includes(normalize(reservation.status))) candidates.push({ reservation, step, requisition, line });
        });
    }
    return candidates;
}
...
export function MatFlowProcessingPage() {
    ...
    const load = useCallback(async () => { setLoading(true); setError(""); try { const [j, c] = await Promise.all([matflowApi.listProcessingJobs(), discoverProcessingCandidates()]); setJobs(Array.isArray(j?.data) ? j.data : []); setCandidates(c); } catch (e) { setError(readMatFlowError(e, "Unable to load Processing workspace.")); } finally { setLoading(false); } }, []);
    useEffect(() => { load(); }, [load]);
