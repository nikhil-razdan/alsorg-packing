...
function PlantSelect() {
    const { availablePlants, selectedPlantCode, canViewAllPlants, setSelectedPlantCode } = useMatFlow();
    if (!canViewAllPlants && availablePlants.length <= 1) return null;
    return (
        <TextField
            select
            value={selectedPlantCode}
            onChange={(event) => setSelectedPlantCode(event.target.value)}
            sx={{ minWidth: 136, "& .MuiOutlinedInput-root": { height: 44 }, "& .MuiSelect-select": { py: 1, fontWeight: 600 } }}
...
function ManagementShell({ view, onViewChange, children, unreadCount = 0 }) {
    const navigate = useNavigate();
    const { user } = useAuth();
    const initials = String(user?.name || user?.username || "AK")
        .split(/\s+/)
        .filter(Boolean)
    ...
    return (
        <ThemeProvider theme={managementTheme}>
            <ScopedCssBaseline>
                <Box sx={{ minHeight: "100vh", bgcolor: C.page, color: C.text }}>
                    <Box
                        component="header"
                        sx={{
                            height: 68,
                            ...
                            display: "grid",
                            gridTemplateColumns: { xs: "1fr auto", md: "290px 1fr 220px" },
                            alignItems: "stretch",
                            px: { xs: 2, md: 3.5 },
                            position: "sticky",
                            top: 0,
                    ...
                    >
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1.6, minWidth: 0 }}>
                            <Typography sx={{ fontSize: 27, lineHeight: 1, fontWeight: 900, letterSpacing: "-.04em", color: C.primaryDark }}>alsorg</Typography>
                            <Box sx={{ width: "1px", height: 30, bgcolor: "#9aa8b8" }} />
                            <Typography sx={{ fontSize: 24, lineHeight: 1, fontWeight: 750, color: C.text }}>MatFlow</Typography>
                        </Box>

                        <Box component="nav" sx={{ display: { xs: "none", md: "flex" }, alignItems: "stretch", justifyContent: "center", gap: 1.5 }}>
                        ...
                        <Box sx={{ display: "flex", justifyContent: "flex-end", alignItems: "center", gap: 1.2 }}>
                            <Tooltip title="Work notifications">
                                <Button
                                    onClick={() => navigate("/matflow/work")}
                                    sx={{ minWidth: 40, width: 40, height: 40, p: 0, color: C.text, borderRadius: "50%" }}
                                >
                                    <Badge color="error" badgeContent={Math.min(99, Number(unreadCount || 0))} invisible={!unreadCount}>
                            ...
                            <Button
                                ...
                                sx={{ minWidth: 42, width: 42, height: 42, p: 0, borderRadius: "50%", bgcolor: "#eef1f4", color: C.text, fontWeight: 800, "&:hover": { bgcolor: "#e4e9ee" } }}
                            >
                                {initials}
                            </Button>
                        </Box>
                    ...
                    <Box sx={{ display: { xs: "flex", md: "none" }, px: 1.5, borderBottom: `1px solid ${C.border}`, overflowX: "auto", bgcolor: C.page }}>
                        {NAV_TABS.map(([key, label]) => {
                            const active = view === key || (view === "work" && key === "overview");
                            return (
                                <Button key={key} onClick={() => onViewChange(key)} sx={{ flex: "0 0 auto", borderRadius: 0, color: active ? C.primary : C.text, borderBottom: active ? `2px solid ${C.primary}` : "2px solid transparent" }}>
                                    {label}
                                </Button>
                            );
                        })}
                    ...
                    <Box component="main" sx={{ width: "100%", maxWidth: 1536, mx: "auto", px: { xs: 2, md: 3.2 }, py: { xs: 2.5, md: 3.5 } }}>
                        {children}
                    </Box>
                </Box>
            </ScopedCssBaseline>
...
export default function MatFlowManagementDashboardPage() {
    const navigate = useNavigate();
    const [searchParams, setSearchParams] = useSearchParams();
    const { selectedPlantParam } = useMatFlow();
    const rawView = clean(searchParams.get("view") || "overview").toLowerCase();
    const view = ["overview", "operations", "projects", "materials", "work"].includes(rawView) ? rawView : "overview";
    ...
    useEffect(() => {
        let active = true;
        const loadNotifications = () => matflowWorkApi.notifications({ plantCode: selectedPlantParam || undefined, limit: 30 })
            .then((response) => { if (active) setNotifications(response?.data || { unreadCount: 0, notifications: [] }); })
            .catch(() => { if (active) setNotifications({ unreadCount: 0, notifications: [] }); });
        loadNotifications();
    ...
    return (
        <ManagementShell view={view} onViewChange={changeView} unreadCount={notifications?.unreadCount || 0}>
...
export { MatFlowManagementDashboardPage };
