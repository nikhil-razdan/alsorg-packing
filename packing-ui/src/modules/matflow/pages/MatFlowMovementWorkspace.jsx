...
export function MatFlowTransfersPage() {
    ...
    return <Box sx={pageSx}>
        <PageHero badge="TRANSFER CONTROL DESK" title="Material Transfers" subtitle="Dispatch and receive reserved material through the approved Store → QC/Processing → Production route." actions={<><Button startIcon={<FileDownloadOutlinedIcon />} onClick={() => downloadMatFlowExcel({ fileName: "MatFlow_Material_Transfers", sheetName: "Transfers", title: "MatFlow Material Transfers", rows: filtered })} sx={secondaryBtnSx}>Export Excel</Button><Button startIcon={<RefreshOutlinedIcon />} onClick={load} sx={secondaryBtnSx}>Refresh</Button></>} />
        <ErrorBox>{error}</ErrorBox>
        <Box sx={{ display: "grid", gridTemplateColumns: "repeat(4,minmax(0,1fr))", gap: 1 }}><SummaryCard label="Ready" tone="sky" value={counts.ready} /><SummaryCard label="In Transit" tone="blue" value={counts.transit} /><SummaryCard label="Partial Receipt" tone="amber" value={counts.partial} /><SummaryCard label="Received" tone="green" value={counts.received} /></Box>
        ...
        <Card sx={panelSx}>{loading ? <LoadingBlock /> : <Box sx={tableShellSx}><Box sx={{ ...tableHeaderSx, gridTemplateColumns: "170px 180px 180px 180px 100px 100px 160px 100px" }}>{["Transfer", "Project / Requisition", "Material", "Route", "Planned", "Received", "Status", "Action"].map((h) => <Box key={h} sx={tableCellSx}>{h}</Box>)}</Box>{filtered.length === 0 ? <EmptyState /> : transferPagination.pageItems.map((row) => <Box key={row.id} sx={{ ...tableRowSx, gridTemplateColumns: "170px 180px 180px 180px 100px 100px 160px 100px" }}><Box sx={tableCellSx}><Typography sx={mainTextSx}>{row.transferNumber || "-"}</Typography><Typography sx={subTextSx}>{row.purpose || "-"}</Typography></Box><Box sx={tableCellSx}><Typography sx={mainTextSx}>{row.projectCode || row.requisitionNumber || "-"}</Typography><Typography sx={subTextSx}>{row.drawingNo || row.requisitionNumber || "-"}</Typography></Box><Box sx={tableCellSx}><Typography sx={mainTextSx}>{row.materialName || "-"}</Typography><Typography sx={subTextSx}>{row.materialCode || "-"}</Typography></Box><Box sx={tableCellSx}>{row.fromLocationCode || "-"} → {row.toLocationCode || "-"}</Box><Box sx={tableCellSx}>{formatQty(row.plannedQty)}</Box><Box sx={tableCellSx}>{formatQty(row.receivedQty)}</Box><Box sx={tableCellSx}><MatFlowStatusChip status={row.status} /></Box><Box sx={tableCellSx}><Button onClick={() => navigate(`/matflow/transfers/${row.id}`)} sx={secondaryBtnSx}>Open</Button></Box></Box>)}</Box>}
            {!loading && (
                <MatFlowPagination
                    {...transferPagination}
...
export function MatFlowReturnsPage() {
    ...
    const confirmDeleteDraft = async () => {
        ...
        try {
            ...
            setError(readMatFlowError(requestError, "Unable to delete the Draft material return."));
        } finally {
            setWorking(false);
        }
    };
    ...
    return <Box sx={pageSx}>
        <PageHero badge="MATERIAL RETURN CONTROL" title="Material Returns" subtitle="Return unused issued material from Production through controlled stock movement." actions={<><Button startIcon={<FileDownloadOutlinedIcon />} onClick={() => downloadMatFlowExcel({ fileName: "MatFlow_Material_Returns", sheetName: "Returns", title: "MatFlow Material Returns", rows })} sx={secondaryBtnSx}>Export Excel</Button><Button startIcon={<RefreshOutlinedIcon />} onClick={load} sx={secondaryBtnSx}>Refresh</Button>{canCreateReturn && <Button startIcon={<AddIcon />} onClick={() => setDialog(true)} sx={primaryBtnSx}>Create Return</Button>}</>} />
        <ErrorBox>{error}</ErrorBox>
        <Card sx={panelSx}>{loading ? <LoadingBlock /> : <Box sx={tableShellSx}><Box sx={{ ...tableHeaderSx, gridTemplateColumns: "170px 170px 180px 150px 160px 220px" }}>{["Return", "Requisition", "Route", "Reason", "Status", "Action"].map((h) => <Box key={h} sx={tableCellSx}>{h}</Box>)}</Box>{rows.length === 0 ? <EmptyState /> : returnPagination.pageItems.map((row) => <Box key={row.id} sx={{ ...tableRowSx, gridTemplateColumns: "170px 170px 180px 150px 160px 220px" }}><Box sx={tableCellSx}>{row.returnNumber}</Box><Box sx={tableCellSx}>{row.requisitionNumber}</Box><Box sx={tableCellSx}>{row.fromLocationCode} → {row.toLocationCode}</Box><Box sx={tableCellSx}>{readable(row.reason)}</Box><Box sx={tableCellSx}><MatFlowStatusChip status={row.status} /></Box><Box sx={{ ...tableCellSx, display: "flex", gap: .5 }}>{normalize(row.status) === "DRAFT" && canRoleActAtLocationType(hasRole, locations.find((l) => String(l.id) === String(row.fromLocationId))?.locationType) && <Button disabled={working} onClick={() => act(row, "dispatch")} sx={primaryBtnSx}>Dispatch</Button>}{normalize(row.status) === "DRAFT" && canCreateReturn && row.rowVersion != null && <Button startIcon={<DeleteOutlineIcon />} disabled={working} onClick={() => setDeleteTarget(row)} sx={dangerBtnSx}>Delete</Button>}{["IN_TRANSIT", "PARTIALLY_RECEIVED"].includes(normalize(row.status)) && canRoleActAtLocationType(hasRole, locations.find((l) => String(l.id) === String(row.toLocationId))?.locationType) && <Button disabled={working} onClick={() => act(row, "receive")} sx={primaryBtnSx}>Receive</Button>}</Box></Box>)}</Box>}
            {!loading && (
                <MatFlowPagination
                    {...returnPagination}
