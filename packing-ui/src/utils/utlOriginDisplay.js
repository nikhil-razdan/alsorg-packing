...
export const fetchUtlOriginMetadataForRows =
  async (
    ...
    for (
      ...
      /*
       * UTL origin metadata is presentation-only. If the caller has already
       * cancelled a stale refresh, return whatever was resolved so far instead
       * of allowing an AbortError to escape into React's effect lifecycle.
       */
      if (signal?.aborted) {
        return result;
      }
